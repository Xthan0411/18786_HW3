...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncate_len = random.randint(min_len, max_len)
        document = document[:truncate_len]

        # 2. Break into substrings: [prefix] [masked_content] [suffix]
        total_len = len(document)
        mask_len = random.randint(0, total_len // 2)
        ...
        # 3. Rearrange: [prefix] MASK [suffix] MASK [masked_content] [pads]
        # The total length needs to be block_size + 1 (for x and y of length block_size)
        pad_len = self.block_size + 1 - total_len - 2
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + self.PAD_CHAR * pad_len

        # 4. Construct input (x) and output (y)
        x_string = masked_string[:-1]
        y_string = masked_string[1:]
